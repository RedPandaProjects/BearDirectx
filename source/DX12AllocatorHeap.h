...
{
	BEAR_CLASS_WITHOUT_COPY(DX12AllocatorHeap);
public:
	DX12AllocatorHeap() {}
...
#ifdef DEBUG
			BEAR_ASSERT(HeapsOfAddres[i]->MaxSize == elements_in_heap);
#endif
		}
	...
	{
		BEAR_ASSERT(elements_in_heap >=count_element);
		DX12AllocatorHeapItem result;
		Heap* heap = 0;
		if(HeapsOfMaxSize.size())
		{
			auto item = bear_lower_bound(HeapsOfMaxSize.begin(), HeapsOfMaxSize.end(), count_element, [](Heap* a1, bsize a2)->bool {return a1->MaxSize < a2; });
		...
		bear_fill(&heap->BlockInfo[id], count_element, 1);
	...
	inline void free(DX12AllocatorHeapItem& Item)
	{
		...
		auto item = bear_lower_bound(HeapsOfAddres.begin(), HeapsOfAddres.end(), Item, [](BearRef<Heap>& a1, DX12AllocatorHeapItem& a2)->bool {return a1->DescriptorHeap.Get() < a2.DescriptorHeap.Get(); });
		BEAR_ASSERT(item != HeapsOfAddres.end());
		BEAR_ASSERT((*item)->DescriptorHeap.Get() == Item.DescriptorHeap.Get());
...
private:
	...
	inline Heap* create_block()
	{
		...
		bear_fill(heap->BlockInfo, elements_in_heap);
		heap->MaxSize = elements_in_heap;
		auto item1 = bear_lower_bound(HeapsOfAddres.begin(), HeapsOfAddres.end(), heap, [](BearRef<Heap>&a1, Heap* a2)->bool {return a1->DescriptorHeap.Get() < a2->DescriptorHeap.Get(); });
		HeapsOfAddres.insert(item1, heap);
		auto item2 = bear_lower_bound(HeapsOfMaxSize.begin(), HeapsOfMaxSize.end(), elements_in_heap, [](Heap* a1, bsize a2)->bool {return a1->MaxSize < a2; });
	...
	BearVector<BearRef<Heap>> HeapsOfAddres;
